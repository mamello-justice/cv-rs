...
impl SobelOperator {
    pub fn new() -> SobelOperator {
        SobelOperator {}
    }
}

impl EdgeDetection for SobelOperator {
    fn detect_edge(&self, image: &GrayImage) -> GrayImage {
        let mut gx_kernel = Kernel::new(3, 3, vec![1, 0, -1, 2, 0, -2, 1, 0, -1]);
        gx_kernel.padding = PaddingType::Zero;

        let mut gy_kernel = Kernel::new(3, 3, vec![1, 2, 1, 0, 0, 0, -1, -2, -1]);
